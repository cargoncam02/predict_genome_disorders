...
```{r tabla2, echo=FALSE}
knitr::kable(head(mod_data %>% 
                    select(8:14)), align="c", format="html", col.names = c("Edad (Padre)","Estado","Frecuencia Respiratoria","Pulsaciones por minuto", "Nivel de riesgo","Género","Asfixia postnatal" )) %>% 
  kable_styling(bootstrap_options =c("bordered", "responsive", "hover"), full_width = F, font_size = 12)
...
```{r tabla3, echo=FALSE}
knitr::kable(head(mod_data %>% 
                    select(15:21)), align="c", format="html", col.names = c("Defectos genéticos en Autopsia", "Detalles ácido fólico", "Enfermedades maternas graves","Exposición a la radiación (rayos X)", "Abuso de sustancias", "Concepción asistida", "Anomalías en embarazos previos")) %>% 
  kable_styling(bootstrap_options =c("bordered", "responsive", "hover"), full_width = F, font_size = 12)
...
```{r tabla4, echo=FALSE}
knitr::kable(head(mod_data %>% 
                    select(22:27)), align="c", format="html",col.names = c("No. abortos previos", "Defectos de nacimiento", "Conteo glóbulos blancos", "Resultados Test sanguíneo", "Tipo Desorden genético/enfermedad", "Subclase de desorden")) %>% 
  kable_styling(bootstrap_options =c("bordered", "responsive", "hover"), full_width = F, font_size = 12)
...
mod_data %>% 
 filter(Birth.defects !="") %>%
  ...
  labs(x="Edad de la madre", y="Defectos de nacimiento", caption = "No se han tenido en cuenta los valores nulos de las variables.", fill = "Tipos de desorden genético:") +
...
filter(Disorder.Subclass !="") %>%
  ...
  labs(x="Conteo glóbulos blancos", y="Frecuencia", caption = "No se han tenido en cuenta los valores nulos de las variables.", fill="Desórdenes genéticos") +
...
```{r grafica4, echo=TRUE, message=FALSE, warning=FALSE}
mod_data %>% 
  ggplot(aes(x = White.Blood.cell.count..thousand.per.microliter., y = Blood.cell.count..mcL., col = "steelblue")) +
  geom_jitter(alpha = 0.5 ,width = 0.4, show.legend = FALSE) +
  geom_smooth(method = "lm", se = TRUE, col = "darkgreen") +
  ggtitle("Distribución de defectos de nacimiento según la edad de la madre") +
  labs(x="Conteo de glóbulos blancos", y="Conteo de células sanguíneas", caption = "No se han tenido en cuenta los valores nulos de las variables.") +
  theme(legend.position= "right", axis.line = element_line(linewidth = 1, colour = "gold"))
